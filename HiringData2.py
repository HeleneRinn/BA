...
for i in range(300):
    index = i + 7000
    gender = random.choices(['Male', 'Female'], weights=[74, 26])[0]
    ...
    experience = int(np.random.lognormal(mean=2.0, sigma=1.0))
    ...
    def generate_salary_expectation(education, experience):
        if education == 'Bachelor':
            ...
            salary_expectation = random.randint(550, 800) * 100
        else:
            salary_expectation = random.randint(650, 900) * 100
    ...
    if not wrong_skills:
        if gender == 'Male':
            if adskills:
                ...
                status = random.choices(['eingestellt', 'ausstehend', 'abgelehnt'], weights=[20, 70, 10])[0]
        if gender == 'Female':
            if adskills:
                status = random.choices(['eingestellt', 'ausstehend', 'abgelehnt'], weights=[0, 30, 70])[0]
            else:
                status = random.choices(['eingestellt', 'ausstehend', 'abgelehnt'], weights=[5, 60, 35])[0]
...
ds.to_csv('hiring_Data_USA.csv', index=False)