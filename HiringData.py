...
for i in range(7000):
    ...
    experience = int(np.random.lognormal(mean=2.1, sigma=1.0))
    ...
    def generate_salary_expectation(education, experience):
        if education == 'Bachelor':
            ...
            salary_expectation = random.randint(600, 900) * 100
        else:
            salary_expectation = random.randint(800, 1100) * 100
    ...
    if not wrong_skills:
        if gender == 'Male':
            if adskills:
                ...
                status = random.choices(['eingestellt', 'ausstehend', 'abgelehnt'], weights=[35, 60, 0])[0]
        if gender == 'Female':
            if adskills:
        ...
        if education == 'PHD' or status == 'eingestellt':
            hired = True
        elif status == 'ausstehend' and wrong_language == False:
            if gender == 'Male':
                if 1.0 <= grade <= 2.9:
                    hired = True
                elif experience > 2:
...
ds.to_csv('hiring_Data_Ger.csv', index=False)
