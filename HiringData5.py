...
for i in range(150):
    index = i + 7450
    gender = random.choices(['Male', 'Female'], weights=[82, 18])[0]
    first_name = fake.first_name_male() if gender == 'Male' else fake.first_name_female()
    last_name = fake.last_name()
    education = random.choices(['Bachelor', 'Master', 'PhD'], weights=[60, 30, 10])[0]
    grade = round(np.random.normal(loc=2.3, scale=0.4), 1)
    grade = max(min(grade, 3.7), 1.0)
    experience = int(np.random.lognormal(mean=2.0, sigma=0.8))
...
ds.to_csv('hiring_Data_Tur.csv', index=False)